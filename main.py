...
RECURSIVE = True  # Walk subdirectories if True
...
# List of category names and flattened prompts for tokenization
...
# Build full paths for each category folder (unused later but kept for reference)
newfile = []
for file in file:
...
# Collect image file paths and names
path_list = []
output_names = []
output_file_names = []
...
# Iterate over each image, encode with CLIP, and print classification probabilities
for path in path_list:
    ...
    print(
        f"the {path} Label probs:{probs} Max prob:{max_prob} Max index:{promt[max_index]}"
    )
